...
def fmtDD():
    ...
    html = etree.HTML(text)
    title=html.xpath('//a[@class="pic" and @dd_name="单品图片"]')
...
for b in cols:
    ...
    if(count%10==0):
        time.sleep(2)
    dd_url='http://search.dangdang.com/?key={0}&act=input'.format(b)
    ddurl=fmtDD()
...
for o in bookList:
    ...
    sheet.write(x, y+2, o.get('ddurl'))
    sheet.write(x, y+3, o['chsTitle'])
    x=x+1
